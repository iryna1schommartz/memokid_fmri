function glm_lss(which_sub, ses_nbr)
...
% approach.
%hhlr=0;
% Modified by Iryna Schommartz 1.03.2022
tic
...
% Main folder
main_folder= '/.../.../.../.../';
addpath('/.../.../.../_common_software/spm8')


% Loop through participants
for cSub = which_sub
    sub_code = sprintf('sub-%d', cSub);
fprintf('Starting participant %d',sub_code)
%%
% Create output folder to keep things nice and clean
outputdir=[main_folder, 'rsa/', sub_code, '/ses-0', num2str(ses_nbr), '/lss/'];
  
% SPM.mat folder
spm_folder= [main_folder, 'rsa/', sub_code, '/ses-0', num2str(ses_nbr), '/betas/'];
    %% MAIN CODE
    % Load pre-existing SPM file containing model information
    fprintf('\nLoading previous model for %s:\n%s\n', sub_code, spm_folder);
    if exist([spm_folder, '/SPM.mat'],'file')
        OrigSpm = load([spm_folder, '/SPM.mat']);
    else
    ...
    for cRun = 1:length(OrigSpm.SPM.Sess)
        ...
        parfor cCond = 1:length(originalNames)% Turn this one into a parfor
            ...
            for cRep = 1:length(lssOnsets{cCond}) 
                ...
                currinfo = {cRun curr_cond cRep length(OrigSpm.SPM.Sess(cRun).U(cCond).ons(cRep)) curr_name trialdir ['RUN' num2str(cRun) '_' curr_name '.nii']};
                ...
                if estimate
                    ...
                    elseif strfind(betaFile, '.nii')
                        system(['cp ' trialdir '/' betaFile ' ' betaDir betaFileName, '.nii']);
                    end
